...
ytofu resource for managing an AWS Network Firewall TLS Inspection Configuration.
...
```bash
ytofu import aws_networkfirewall_tls_inspection_configuration.example arn:aws:network-firewall::<region>:<account_id>:tls-configuration/example
```
