...
- `graph_name` (String, Forces new resource) Contains a user-supplied name for the Graph. If omitted, ytofu will assign a random, unique identifier.
...
- `tags` - (Optional) Key-value tags for the graph. If configured with a provider `default_tags` configuration block present, tags with matching keys will overwrite those defined at the provider-level.
...
```bash
ytofu import aws_neptunegraph_graph.example "graph_id"
```
