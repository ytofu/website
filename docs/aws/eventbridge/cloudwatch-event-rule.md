...
* `name` - (Optional) The name of the rule. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`.
...
```bash
ytofu import aws_cloudwatch_event_rule.example example-event-bus/capture-console-sign-in
```
