...
* `input_template` - (Required) Template to customize data sent to the target. Must be valid JSON. To send a string value, the string value must include double quotes. Values must be escaped for both JSON and ytofu, e.g., `"\"Your string goes here.\\nA new line.\""`
...
```bash
ytofu import aws_cloudwatch_event_target.example rule-name/target-id
```
