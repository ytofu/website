...
* `name` - (Optional, Forces new resource) Name of the schedule. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`.
...
```bash
ytofu import aws_scheduler_schedule.example my-schedule-group/my-schedule
```
