...
ytofu resource for managing an AWS EventBridge Pipes Pipe.
...
* `name` - (Optional) Name of the pipe. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`.
...
```bash
ytofu import aws_pipes_pipe.example my-pipe
```
