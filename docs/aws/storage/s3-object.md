...
* `etag` - (Optional) Triggers updates when the value changes. The only meaningful value is `filemd5("path/to/file")` (ytofu 0.11.12 or later) or `${md5(file("path/to/file"))}` (ytofu 0.11.11 or earlier). This attribute is not compatible with KMS encryption, `kms_key_id` or `server_side_encryption = "aws:kms"`, also if an object is larger than 16 MB, the AWS Management Console will upload or copy that object as a Multipart Upload, and therefore the ETag will not be an MD5 digest (see `source_hash` instead).
...
* `source_hash` - (Optional) Triggers updates like `etag` but useful to address `etag` encryption limitations. Set using `filemd5("path/to/source")` (ytofu 0.11.12 or later). (The value is only stored in state and not saved by AWS.)
...
* `default_tags` - (Optional) Override the provider `default_tags` configuration block.
...
```bash
ytofu import aws_s3_object.example some-bucket-name/some/key.txt
```
