...
* `etag` - (Optional) Triggers updates when the value changes. The only meaningful value is `filemd5("path/to/file")` (ytofu 0.11.12 or later) or `${md5(file("path/to/file"))}` (ytofu 0.11.11 or earlier). This attribute is not compatible with KMS encryption, `kms_key_id` or `server_side_encryption = "aws:kms"` (see `source_hash` instead).
...
```bash
ytofu import aws_s3_bucket_object.example some-bucket-name/some/key.txt
```
