...
Further more, the `name`, `description`, `cors_configuration`, `tags` and `version` fields should be specified in the ytofu configuration and the values will override any values specified in the OpenAPI document.
...
```bash
ytofu import aws_apigatewayv2_api.example aabbccddee
```
