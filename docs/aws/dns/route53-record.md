...
* `records` - (Required for non-alias records) A string list of records. To specify a single record value longer than 255 characters such as a TXT record for DKIM, add `\"\"` inside the ytofu configuration string (e.g., `"first255characters\"\"morecharacters"`).
...
* `allow_overwrite` - (Optional) Allow creation of this record in ytofu to overwrite an existing record, if any. This does not affect the ability to update the record in ytofu and does not prevent other resources within ytofu or manual Route 53 changes outside ytofu from overwriting this record. `false` by default. This configuration is not recommended for most environments.
...
* `zone_id` - (Required) Hosted zone ID for a CloudFront distribution, S3 bucket, ELB, AWS Global Accelerator, or Route 53 hosted zone. See `resource_elb.zone_id` for example.
...
```bash
ytofu import aws_route53_record.example Z4KAPRWWNC7JR_dev_NS
```
