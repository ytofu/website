...
* `name` - (Optional, Forces new resource) Name of the instance profile. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`. Can be a string of characters consisting of upper and lowercase alphanumeric characters and these special characters: `_`, `+`, `=`, `,`, `.`, `@`, `-`. Spaces are not allowed. The `name` must be unique, regardless of the `path` or `role`. In other words, if there are different `role` or `path` values but the same `name` as an existing instance profile, it will still cause an `EntityAlreadyExists` error.
...
```bash
ytofu import aws_iam_instance_profile.test_profile app-instance-profile-1
```
