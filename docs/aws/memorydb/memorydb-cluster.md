...
* `description` - (Optional) Description for the cluster. Defaults to `"Managed by ytofu"`.
...
* `name` - (Optional, Forces new resource) Name of the cluster. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`.
...
```bash
ytofu import aws_memorydb_cluster.example my-cluster
```
