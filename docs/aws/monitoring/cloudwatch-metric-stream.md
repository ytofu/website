...
* `name` - (Optional, Forces new resource) Friendly name of the metric stream. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`.
...
```bash
ytofu import aws_cloudwatch_metric_stream.sample sample-stream-name
```
