...
`aws_codebuild_webhook` resource, which manages the webhook to the
...
* `id` - Name (if imported via `name`) or ARN (if created via ytofu or imported via ARN) of the CodeBuild project.
...
```bash
ytofu import aws_codebuild_project.name project-name
```
