...
* `name` -  (Optional) A custom name for the job. The name can contain as many as 500 characters. If omitted, ytofu will assign a random, unique name. Conflicts with `name_prefix`.
...
```bash
ytofu import aws_macie2_classification_job.example abcd1
```
