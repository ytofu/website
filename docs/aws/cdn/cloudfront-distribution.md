...
* `retain_on_delete` (Optional) - Disables the distribution instead of deleting it when destroying the resource through ytofu. If this is set, the distribution needs to be deleted manually afterwards. Default: `false`.
...
```bash
ytofu import aws_cloudfront_distribution.distribution E74FTE3EXAMPLE
```
