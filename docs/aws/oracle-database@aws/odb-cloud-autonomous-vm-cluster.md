...
ytofu resource managing cloud autonomous vm cluster in AWS for Oracle Database@AWS.
...
* `cloud_exadata_infrastructure_id` - (Optional) Exadata infrastructure id. Changing this will force ytofu to create a new resource. Either the combination of `cloud_exadata_infrastructure_id` and `odb_network_id` or `cloud_exadata_infrastructure_arn` and `odb_network_arn` must be used.
* `cloud_exadata_infrastructure_arn` - (Optional) Exadata infrastructure ARN. Changing this will force ytofu to create a new resource. Either the combination of `cloud_exadata_infrastructure_id` and `odb_network_id` or `cloud_exadata_infrastructure_arn` and `odb_network_arn` must be used.
* `odb_network_id` - (Optional) Unique identifier of the ODB network associated with this Autonomous VM Cluster. Changing this will force ytofu to create a new resource. Changing this will create a new resource. Either the combination of `cloud_exadata_infrastructure_id` and `odb_network_id` or `cloud_exadata_infrastructure_arn` and `odb_network_arn` must be used.
* `odb_network_arn` - (Optional) ARN of the ODB network associated with this Autonomous VM Cluster. Changing this will force ytofu to create a new resource. Either the combination of `cloud_exadata_infrastructure_id` and `odb_network_id` or `cloud_exadata_infrastructure_arn` and `odb_network_arn` must be used.
...
```bash
ytofu import aws_odb_cloud_autonomous_vm_cluster.example example
```
