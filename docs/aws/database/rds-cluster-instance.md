...
* `cluster_identifier` - (Required, Forces new resource) Identifier of the `aws_rds_cluster` in which to launch this instance.
...
* `identifier` - (Optional, Forces new resource) Identifier for the RDS instance, if omitted, ytofu will assign a random, unique identifier.
...
```bash
ytofu import aws_rds_cluster_instance.prod_instance_1 aurora-cluster-instance-1
```
