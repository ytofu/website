...
Manages an RDS database cluster snapshot copy. For managing RDS database instance snapshot copies, see the `aws_db_snapshot_copy` resource.
...
```bash
ytofu import aws_rds_cluster_snapshot_copy.example my-snapshot
```
