...
accounts is enabled.
* `identifier` - (Optional) The name of the RDS instance, if omitted, ytofu will assign a random, unique identifier. Required if `restore_to_point_in_time` is specified.
...
```bash
ytofu import aws_db_instance.default mydb-rds-instance
```
