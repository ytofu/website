...
* `kubernetes_network_config` - (Optional) Configuration block with kubernetes network configuration for the cluster. [Detailed](#kubernetes_network_config) below. If removed, ytofu will only perform drift detection if a configuration value is provided.
...
* `public_access_cidrs` - (Optional) List of CIDR blocks. Indicates which CIDR blocks can access the Amazon EKS public API server endpoint when enabled. EKS defaults this to a list with `0.0.0.0/0`. ytofu will only perform drift detection of its value when present in a configuration.
...
```bash
ytofu import aws_eks_cluster.my_cluster my_cluster
```
