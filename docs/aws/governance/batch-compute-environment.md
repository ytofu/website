...
* `name` - (Optional, Forces new resource) The name for your compute environment. Up to 128 letters (uppercase and lowercase), numbers, and underscores are allowed. If omitted, ytofu will assign a random, unique name.
...
```bash
ytofu import aws_batch_compute_environment.sample sample
```
